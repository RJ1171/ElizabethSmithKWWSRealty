...
- `package.json` - static-only package file with no docs dependency

## Important: No docs.json

This project does not use `docs.json`, Next.js, a docs template, or any framework build dependency.

If your Vercel deployment is failing because `docs.json` is missing, your repo is still using an old template file or build command. Replace the repo contents with this static site package, or remove the old framework/package files from the repo.
